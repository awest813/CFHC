...
| 5 — Test hardening | ✅ Done — desktop-standalone gate now runs the `positions/` + `staff/` suites (19 files, previously Android-CI-only); `nightly.yml` runs the stress suites daily (`-PrunStress`) plus the SDK-free gate; `bumpVersion` syncs all three version locations atomically (validated before write; v1.4.N → 1.4+Nth letter). |
| 6 — Release checklist | ✅ Done — **v1.4.5 published**: CI signing secrets configured; tag `v1.4.5` → release workflow built and attached signed `app-release.aab` + `app-release.apk` (signature verified: `CN=CFHC Release`, SHA-256 `864518a4…` — the real keystore, not debug) and `CFHC-desktop-1.4e.jar`. In-app updater verified live: `DesktopUpdateChecker.check()` against the real GitHub API returns `UP_TO_DATE` for a current 1.4e build. En route it caught and fixed a false-positive: `isSameVersion` now maps Android patch tags to the desktop letter scheme (tested). Remaining manual items (Play Console upload, optional portable zip / MSI / DMG) are in `docs/release-runbook.md` §3–5. |
